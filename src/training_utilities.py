...
def fit_ae_decoder(model, mode=None, tr_data=None, val_data=None, num_epochs=10, bs=32, lr=0.1, momentum=0., **kwargs):
    """
    Training functions for the AEs
    :param model: model to train
    :param mode: (str) {'basic | 'contractive' | 'denoising'}
    ...
    # set optimizer, loss type and datasets (depending on the type of AE)
    optimizer = torch.optim.SGD(model.decoder.parameters(), lr=lr, momentum=momentum)
    criterion = ContractiveLoss(ae=model, lambd=1e-4) if mode == 'contractive' else nn.MSELoss()
    if mode == 'denoising':
        if tr_data is not None or val_data is not None:
            warnings.warn("'denoising' flag was set, so NoisyMNIST will be used for training and validation")
...
def fit_classifier(model, mode=None, tr_data=None, val_data=None, num_epochs=10, bs=32, lr=0.1, momentum=0., **kwargs):
    """
    Training functions for the AEs
    :param model: model to train
    :param mode: (str) {'basic | 'contractive' | 'denoising'}
    ...
    if tr_data is None:
        tr_data, tr_targets = tr_set.data, tr_set.labels  # FastMNIST の tr_set.target は元画像と同じものが入っているので、 .labels に変える
    else:
        assert False

    if val_data is None:
        val_data, val_targets = val_set.data, val_set.labels
    else:
        assert False
